    ...
    if toolchain_version != RUST_TOOLCHAIN:
        ...
        try:
            ...
            for folder in os.listdir( toolchain_dir ):
                shutil.move( p.join( toolchain_dir, folder ),
                            RUST_ANALYZER_DIR )
    ...
    try:
        ...
        java_version = int(
          subprocess.check_output(
            [ java, os.path.join( DIR_OF_THIS_SCRIPT,
...
def GetClangdTarget():
    if OnWindows():
        return [
          ( 'clangd-{version}-win64',
            'aca9cf6d924910145b6e81aea36441fa60d3b9af119ba2b386d00168bccc2cf1' ),
          ( 'clangd-{version}-win32',
            '4675dfd8c3b73bdfb82377051f890e6f122e41e16883d4e33e9491b5860dda6a' ) ]
    if OnMac():
        return [
          ( 'clangd-{version}-x86_64-apple-darwin',
            'ab1fb6694e698d9663f391ecf2c9fcc6feac990593fe0c457a67fe057541bb85' ) ]
    if OnFreeBSD():
        return [
    ...
    if OnX86_64(): # Linux
        # os.confstr('CS_GNU_LIBC_VERSION') return "glibc 2.18"
        glibc_version = float(os.confstr('CS_GNU_LIBC_VERSION').split()[1])
        if(glibc_version >= 2.18):
            return [
...
def DownloadClangd( printer ):
    ...
    download_url = 'https://github.com/BlurryLight/llvm/releases/download/10.0.0/{}'.format(
        file_name )
